...
Napi::Value TranscribeAudio(const Napi::CallbackInfo& info) {
    ...
    whisper_context* ctx = init_whisper_context(params, 0, nullptr);

    audio_async audio(params.length_ms);
    if (!audio.init(params.capture_id, WHISPER_SAMPLE_RATE)) {
        Napi::Error::New(env, "Audio initialization failed").ThrowAsJavaScriptException();
        return env.Null();
    }
    audio.resume();
    ...
    while (is_running) {
        ...
        // Append the transcription result to the result string
    ...
    result.Set("text", Napi::String::New(env, result_text));
